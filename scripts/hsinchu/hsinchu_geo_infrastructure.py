...
def clean_infra_name(name):
    """移除建設特徵綴詞與冗言，留下核心地名"""
    for prefix in ["二十五里", "原設", "今設", "新建", "重修", "在"]:
         if name.startswith(prefix):
             name = name[len(prefix):]
             
    # 移除後綴字眼
    res = name.replace('橋', '').replace('崎', '').replace('嶺', '').replace('渡', '').replace('口', '').replace('土城', '').replace('城門', '').replace('隘', '')
...
def main():
    ...
    cursor.execute("""
        ...
        WHERE e.type = 'Infrastructure' AND sl.id IS NULL
    """)
    infras = cursor.fetchall()
    
    print(f"Start matching {len(infras)} Infrastructure entities against spatial bases...")
    
    match_count = 0
    for ent_id, name in infras:
        clean_name = clean_infra_name(name)

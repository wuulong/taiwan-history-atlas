...
## [v260307.2] - 2026-03-07
### 🏛️ 跨區域擴展：新竹五書歷史地名密度分析範例
- **實作新竹 HGIS 探索專案**：
    - **地名提及頻次分析 (Mentions Density)**：首度將「史料文本量化指標」轉化為空間熱點圖（以新竹五書為數據源）。
    - **自動化技能驗證**：透過 `qgis-project-architect` 技能一鍵產製，驗證了自動化引擎在不同區域數據上的重用性。
- **釋出範例**：
    - `examples/hsinchu_hgis_poc/`: 包含新竹歷史重心的 Heatmap 專案、VRT 及地名頻次數據快照。
- **技能組更新**：
    - 正式將 **`qgis-project-architect`** 納入 AI 技能索引，支援 YAML 驅動的軟體定義地圖生成。
...
- **大歷史語義厚化 (L3)**：
    - 實作「斷點續傳」批次作業管線 (`batch_l3_enrichment.py`)。
    - 首波完成曾文溪流域 133 處遺址之「地理歷史脈絡」自動生成，`enrichment_level` 正式晉升。
- **空間拓樸分析 (L4)**：
    - **水系對合**：計算 2,249 處遺址與全台主支流之最近鄰距離與從屬水系。
    - **海拔 Z 軸對應**：引入 20m DTM (數值地形模型)，完成曾文溪流域遺址高程標註。
    - **聚落移轉研究**：產出《曾文溪流域聚落時空移轉深度分析報告》，識別出「海岸 -> 河階 -> 平原」的三部曲變遷。
- **工具釋出**：
    - `scripts/enrich_sites_with_elevation.py`: 地形高程對齊工具。
    - `scripts/analyze_settlement_shift.py`: 聚落地景移轉分析模型。
    - `scripts/batch_l3_enrichment.py`: 語義脈絡批次生成管線 (整合 Fab Ask)。
